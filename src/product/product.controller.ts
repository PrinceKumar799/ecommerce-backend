...
export class ProductController {
  constructor(private readonly productService: ProductService,private readonly reviewService: ReviewService,private readonly cartService:CartService ) {}
  ...
  addReview(@Param('productId') productId: string, @Body() createReviewDto: CreateReviewDto,@Request() req)
  {
    return this.reviewService.create(createReviewDto, req.user.userId, +productId);
  ...
  addToCart(@Param('productId') productId: string, @Request() req,@Body() bodyObj: {quantity: string}) {
    return this.cartService.addItem(req.user.userId, +productId, +bodyObj.quantity);
  }
